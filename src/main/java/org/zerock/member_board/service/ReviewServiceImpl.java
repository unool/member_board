...
public class ReviewServiceImpl implements ReviewService{
    ...
    public Long register(ReviewDTO dto , MultipartFile[] photos) {
        ...
        if(dto.getBno() == null || dto.getWriterEmail() == null)
        {
            throw new ControllerException("null.parameter.values", this.getClass().getName());
        }

        Optional<Board> boardOptional = boardRepository.findById(dto.getBno());
        Optional<Member> memberOptional = memberRepository.findById(dto.getWriterEmail());

        if(!boardOptional.isPresent() || !memberOptional.isPresent())
        {
            throw new ControllerException("not.field.values", this.getClass().getName());
        }

        Board board = boardOptional.get();
        Member member = memberOptional.get();
        Review review  = reviewRepository.save(dtoToEntity(board,member, dto));
    ...
    @Override
    public PageResultDTO<ReviewDTO> getList(PageRequestDTO pageRequestDTO) {
    ...
    public void modify(ReviewDTO reviewDTO, MultipartFile[] photos) {
        ...
        if(result.size() > 0) //애초에 기존 사진이 없다면 삭제할것도 없음.. //하나도 없을때 테스트해볼것
        {
            ...
            if(dtoList != null)
            {
                for(int i = 0; i < dtoList.size(); i++)
                {
                    ...
                    for(int j = 0; j < result.size(); j++)
                    {
                        ...
                        if(ri.getInum() == inum)
                        {
                            existImage[j] = true; //안 지워진 이미지 체크
                        }
                    }
                }
            }
            ...
            {
                if(existImage[i] == false) //지원진 이미지일 경우
                {
                    //이미지 삭제
